...
float heuristic(vector<pair<int, int>> komandosi) {

	return admissible_heuristic(komandosi) * (1 + EPS);
...
int main() {
	...
	typedef pair<float, vector<pair<int, int>>> State;
	...
	while (true) {
		...
		map<vector<pair<int, int>>, float> distance;
