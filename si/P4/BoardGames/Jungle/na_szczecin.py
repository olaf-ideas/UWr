...
class Player(object):
    ...
    def score(self, init_game, start_move, cur_player):
        game = copy.deepcopy(init_game)
        game.update(cur_player, start_move)

        goal = game.dens[cur_player ^ 1]
        
        # print("piece: ", game.pieces[cur_player][6])
        # print("goal: ", goal)
        ...
        for piece in range(8):
            ...
            if piece in game.pieces[cur_player]:
                (ax, ay) = goal
                (bx, by) = game.pieces[cur_player][piece]
    ...
    def simulate(self, init_game, start_move, reps, cur_player):

        # print("xd", cur_player, init_game.curplayer)
        game = copy.deepcopy(init_game)
        game.update(cur_player, start_move)
        player = cur_player ^ 1
        ...
        for _ in range(reps):
            ...
            if moves:
                ...
                result = game.update(player, move)
            else:
                result = game.update(player, None)
    ...
    def loop(self):
        while True:
            ...
            if cmd == 'HEDID':
                ...
                self.game.update(self.my_player ^ 1, move)
            elif cmd == 'ONEMORE':
                self.reset()
                continue
            elif cmd == 'BYE':
            ...
            # print("my player: ", self.my_player)
            # print("game: ", self.game.curplayer)
            ...
            if moves:
                ...
                N = 20000
                # print("S: ", S)
                simulated_S = [(move, self.score(self.game, move, self.my_player)) for move in S]

                def compare(e):
                    return e[1]
                
                S = sorted(simulated_S, key=compare)

                (move, score) = S[0]
                # print("score: ", score)
                # print("move: ", move)

                self.game.update(self.my_player, move)
                move = (move[0][0], move[0][1], move[1][0], move[1][1])
            else:
                self.game.update(self.my_player, None)
