...
let check_rec (defs:(cname * expr) list) : bool = 
  let rec check_expr (e:expr) (idents:cname list) (inf:bool) = 
    match e with
    | Int _ | Bool _ | Unit | Raise -> true
    | Ctor(_,e) | Fst(e) | Snd e-> check_expr e idents inf
    | Fun(_,e ) -> check_expr e idents true
    | Binop(_,e1,e2) |  Let(_,e1,e2) | Pair(e1,e2) | App(e1,e2) | Try(e1,e2) -> check_expr e1 idents inf && check_expr e2 idents inf
    | Var x -> if List.exists (fun y -> y = x) idents then inf else true 
    | If(e1,e2,e3) -> check_expr e1 idents inf && check_expr e2 idents inf && check_expr e3 idents inf
    | Match(e,cl) -> check_expr e idents inf && check_clauses cl idents inf
    | _ -> false
  and check_clauses cl idents inf =
    List.fold_left (fun acc (_,e) -> acc && check_expr e idents inf) true cl
  in
  let idents =  List.map (fun (a,_) -> a) defs in
  List.fold_left (fun acc (_,e) -> (acc && check_expr e idents false)) true defs
...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | Let (x, e1, e2) ->
      ...
      eval_env new_env e2
  | LetRec (defs, e) ->
    if check_rec(defs) =  false then raise MyExn else 
    let (env, defs) =
      List.fold_left_map
        (fun env (x, e) ->
