...
app:
  ...
  | SND; e = base { Snd e }
  | RAISE { Raise }
  | c = CNAME; e = ctor_params { Ctor(c, e) }
  | e = base { e }
  ;

ctor_params:
  | LPAREN; es = separated_list(COMMA, mixfix); RPAREN { es }
  | { [] }
  ;
...
clause:
  | ps=pattern_list; ARR; e=mixfix { (ps, Bool(true), e) }
  | ps=pattern_list; WHEN; e1=expr; ARR; e2=mixfix { (ps, e1, e2) } 
  ;

pattern_list:
  | p=pattern; CON; ps=pattern_list; { p :: ps }
  | p=pattern; { [p] }
...
pattern_app:
  | c = CNAME; ps = cname_list; { PCtor(c, ps) }
  | p=pattern_base { p }
  ;

cname_list:
  | ps = separated_list(COMMA, pattern_base); { ps }
  | LPAREN; RPAREN; { [] }
  ;
