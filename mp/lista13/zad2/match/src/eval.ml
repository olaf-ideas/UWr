...
and match_clauses env v cs =
  ...
  | ([p], e1, e2) :: cs ->
    (match match_pattern env v p with
	| Some env when (eval_env env e1) = VBool(true) -> eval_env env e2
	| Some _ | None -> match_clauses env v cs)
  | (p :: ps, e1, e2) :: cs ->
    (match match_pattern env v p with
	| Some env -> match_clauses env v ((ps, e1, e2) :: cs)
	| None -> match_clauses env v cs)
  | ([], _, _) :: cs' ->
    match_clauses env v cs'
...
let rec string_of_value v =
  ...
  | VCtor(c, vs) -> c ^ "(" ^ string_of_ctor vs ^ ")"

and string_of_ctor es =
  match es with
  | e :: [] -> string_of_value e
  | e :: es -> string_of_value e ^ ", " ^ string_of_ctor es
  | [] -> ""
