...
let rec compile_aexp (env : var option list) (dict : (name * name) list) (a : aexp) : cmd list =
  ...
  | Call (f, args) ->
    ...
    (* Printf.printf "Is there %s in father_dict 1\n" f; *)
    let g = (match List.assoc_opt f dict with
    | Some h -> h
    | None   -> f) in
    (* Printf.printf "Is there %s in father_dict 2\n" f; *)
    let cmds = lookup_for_function_env env (List.assoc g !father_dict) 0 in
...
let rec compile_funs (env : var option list) (dict : (name * name) list) (funs : func list) : (name * cmd list) list =
  ...
  | Func (name, args, local_vars, local_funs, body) :: funs ->
    ...
    Printf.printf "Function (%s) expects env:  " name; print_env new_env;
...
let rec create_father_dict (father : name) (funs : func list) =
  match funs with
  | [] -> ()
  | Func (name, _, _, local_funs, _) :: funs ->
    let local_funs = add_prefix_to_funs (name ^ "_") local_funs in
    Printf.printf "adding to father_dict: %s -> %s\n" name father;
    father_dict := (name, father) :: !father_dict;
    create_father_dict name local_funs;
    create_father_dict father funs;
    ()
