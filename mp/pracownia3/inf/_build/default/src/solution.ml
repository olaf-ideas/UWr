...
let rec compile_funs (env : var option list) (dict : (name * name) list) (funs : func list) : (name * cmd list) list =
  ...
  | [] -> []
  | Func (name, args, local_vars, local_funs, body) :: funs ->
    let new_env = [Some "env_pointer"] @ (List.map Option.some local_vars) @ [None] @ (List.rev (List.map Option.some args)) @ env in
    ...
    let fun_code = compile_stmt new_env body in
...
let compile_prog ((vars, funs, main_stmt) : prog) : vm_prog =
  let env = [Some "env_pointer"] @ List.map Option.some vars in
