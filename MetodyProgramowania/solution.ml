...
let build_row ps n =
    ...
    in
    List.map (fun xs -> List.tl xs) (help ps (n + 1))

let rec build_candidate pss n =
    match pss with
    | [] -> failwith "pss cannot be empty"
    | ps :: [] -> List.map (fun x -> [x]) (build_row ps n)
    | ps :: pss' ->
...
let verify_row ps xs = 
    let rec help xs count =
        ...
        | false :: xs' -> if count = 0 then help xs' 0 else count :: help xs' 0
        | true :: xs' -> help xs' (count + 1)
    in
    ps = (help xs 0)
...
let rec verify_rows pss xss =
    match pss, xss with
    | [], _ | _, [] -> true
    | ps :: pss', xs :: xss' -> (verify_row ps xs) && (verify_rows pss' xss')
...
let transpose xss = 
    let rec help xss res = 
        ...
        | [] -> List.rev res
        | [] :: xss' -> help [] res
        | (x :: xs) :: xss' -> help (List.map List.tl xss) ((x :: (List.map List.hd xss')) :: res)
    in help xss []
