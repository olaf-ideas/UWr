...
with open('konkurs-I-345615-dane.txt', 'r') as data:
	for i in range(lines // 5):
		...
		ts = list(map(float, data.readline().split()))
		us = list(map(float, data.readline().split()))
		data.readline()
		...
		plt.plot([S_x.interpolate(t) for t in us], [-S_y.interpolate(t) for t in us], color='red')
