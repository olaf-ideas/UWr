...
class LineDraggablePoint(DraggablePoint):
    def __init__(self, ax, editor, x, y, color='green', line=None):
        super().__init__(ax, editor, x, y, color)
        self.line = line
    ...
    def on_motion(self, event):
        ...
        closest_index = min(range(len(xs)), key=lambda i: (xs[i] - (x0 + dx))**2 + (ys[i] - (y0 + dy))**2)
        new_x, new_y = xs[closest_index], ys[closest_index]
        self.circle.center = (new_x, new_y)
        self.editor.update_nifs_points()
...
class ImageEditor:
    ...
    def onclick(self, event):
        ...
        if event.button == 1 and not event.key == 'shift':  # Left-click to add
            ...
            for point in self.nifs_points:
                contains, _ = point.circle.contains(event)
                if contains:
                    point.remove()
                    self.nifs_points.remove(point)
                    self.update_nifs_points()
                    return
    ...
    def save_data(self):
        if len(self.points) < 3:
            print("Not enough points to save data.")
            return
        ...
        data_filename = "konkurs-I-<numerindeksu>-dane.txt"
        summary_filename = "konkurs-I-<numerindeksu>-podsumowanie.txt"
        ...
        print(f"Data saved to {data_filename} and {summary_filename}.")

        # Store and reset for new batch
        self.spline_lines.append(self.spline_line)
        self.nifs_lines.append(self.nifs_line)
        self.spline_line = None
        self.nifs_line = None
        self.points = []
        self.nifs_points = []
    ...
    def update_spline(self):
        ...
        self.update_nifs_points(interpolated_xs, interpolated_ys)
        self.fig.canvas.draw()

    def update_nifs_points(self, interpolated_xs=None, interpolated_ys=None):
        if self.nifs_line:
            self.nifs_line.remove()

        if interpolated_xs is None or interpolated_ys is None:
            interpolated_xs, interpolated_ys = zip(*[p.circle.center for p in self.nifs_points])

        self.nifs_line, = self.ax.plot(interpolated_xs, interpolated_ys, color='green')

        if len(self.nifs_points) < 3:
            for x, y in zip(interpolated_xs, interpolated_ys):
                point = LineDraggablePoint(self.ax, self, x, y, line=self.nifs_line)
                self.nifs_points.append(point)
